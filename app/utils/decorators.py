...
def role_required(*roles):
    ...
    def decorator(f):
        ...
        def decorated_function(*args, **kwargs):
            if not current_user.is_authenticated:
                flash(
                    "Ingresa con tu usuario para ver esta página.",
...
def confirmed_required(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        if hasattr(current_user, "is_confirmed") and not current_user.is_confirmed:
            flash(
                "Por favor confirma tu email antes de continuar.", FlashCategory.WARNING
            )
    ...
    def decorated_function(*args, **kwargs):
        try:
            ...
            current_app.logger.error(
                f"Exception in {f.__name__}: {str(e)}", exc_info=True
            )
            flash(
                "Ocurrió un error. Por favor intente nuevamente.", FlashCategory.ERROR
            )
