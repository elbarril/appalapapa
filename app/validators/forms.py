...
from app.utils.constants import MIN_PASSWORD_LENGTH, MAX_NAME_LENGTH, MAX_PRICE, MIN_PRICE
...
class ResetPasswordForm(FlaskForm):
    ...
    security = StringField('Pregunta de Seguridad', validators=[
        DataRequired(message='La respuesta de seguridad es requerida.')
    ])
...
class PersonForm(FlaskForm):
    ...
    notes = TextAreaField('Notas', validators=[
        Optional(),
        Length(max=1000, message='Las notas son demasiado largas (máximo 1000 caracteres).')
    ])
...
class DeletePersonForm(FlaskForm):
    ...
    confirm = BooleanField('Confirmo que deseo eliminar este paciente', validators=[
        DataRequired(message='Debes confirmar para eliminar.')
    ])
...
class SessionForm(FlaskForm):
    ...
    person_id = SelectField('Paciente', coerce=int, validators=[
        DataRequired(message='Selecciona un paciente.')
    ])
    session_date = DateField('Fecha', validators=[
        DataRequired(message='La fecha es requerida.')
    ])
    session_price = DecimalField('Precio', places=2, validators=[
    ...
    notes = TextAreaField('Notas', validators=[
        Optional(),
        Length(max=500, message='Las notas son demasiado largas (máximo 500 caracteres).')
    ])
...
class FilterForm(FlaskForm):
    ...
    show = SelectField('Mostrar', choices=[
        ('all', 'Todos'),
        ('pending', 'Pendientes'),
        ('paid', 'Pagados')
    ], default='all')
